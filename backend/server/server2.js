...
// GET endpoint to retrieve a contract ID - updated to check both locations
app.get('/api/projects/:name/contract-id', async (req, res) => {
  try {
    ...
    // First check for the JSON file in the project root (preferred)
    const contractIdJsonFile = path.join(projectDir, 'contractid.json');
    
    if (fsSync.existsSync(contractIdJsonFile)) {
      try {
        // Read and parse the JSON file
        const jsonContent = await fs.readFile(contractIdJsonFile, 'utf8');
        ...
        console.error('Error reading contractid.json:', jsonError);
        // Continue to check the next location if this one fails
      }
    }
    
    // If JSON file doesn't exist or is invalid, check the .stellar directory
    ...
    if (fsSync.existsSync(contractIdFile)) {
      ...
      return res.json({ success: true, contractId });
    }
...
// Store a contract ID for a project - updated to store in both locations
app.post('/api/projects/:name/contract-id', async (req, res) => {
  try {
    const { name } = req.params;
    const { contractId, network = 'testnet', source = 'unknown' } = req.body;
    ...
    await fs.writeFile(contractIdJsonFile, JSON.stringify(contractData, null, 2));
    
    // Update contracts registry
    try {
      const contractsData = await getContractsFromFile();
      const newContract = {
        name,
      ...
      console.error('Error updating contracts registry:', contractsError);
      // Don't fail the entire operation if contract registry update fails
    }
    
    res.json({ 
      success: true, 
      message: 'Contract ID saved successfully',
      contractData
    });
  } catch (error) {
...
// Invoke a contract function - FIXED VERSION
app.post('/api/contracts/invoke', async (req, res) => {
  try {
    const { contractId, function: functionName, args, source, network } = req.body;
    ...
    // Construct arguments string - FIXED
    let argsString = '';
    if (args && args.length > 0) {
      // If first arg is a string and doesn't include a parameter name (doesn't start with --),
      // add the "--to" prefix which is a common convention for the first parameter
      argsString = args.map((arg, index) => {
        if (typeof arg === 'string') {
          // If the string already includes parameter name (starts with --), use it as is
          if (arg.startsWith('--')) {
            return arg;
          }
          // For the first parameter, we often use --to as the convention
          // For other string parameters, we need to use the appropriate parameter name
          // In a real implementation, we'd get this from contract metadata
          return `--to ${arg}`;
        } else if (arg === null || arg === undefined) {
          return '';
        } else {
          // For numeric values we assume it's a value, not a parameter name
          return `${arg}`;
        }
      }).join(' ');
    }
